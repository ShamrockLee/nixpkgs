...
- The Go package builder `buildGoModule` is refactored to adopt the overlay-style `stdenv.mkDerivation` and to fix overriding. `vendorHash` can now be overridden directly using `overrideAttrs`.
