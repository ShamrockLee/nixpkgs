...
- The build helper `buildLinux` now provides a structural `passthru.tests` for its result Linux kernel packages as an attribute set instead of a list. Additionally, the input argument `kernelTests` has been updated to accept an attribute set by default.

  For compatibility purpose, the previous usage of `kernelTests` as a list is still supported in this release. Test cases specified in this manner is accessible as `passthru.tests.extra-kernel-tests-<0-based index>`.
