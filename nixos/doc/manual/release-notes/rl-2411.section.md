...
- `xrootd` version `5.7.1` no longer produces `bin/test-runner` for test execution. Therefore, the `enableTestRunner` argument are removed from the override interface, and so does the `xrootd.tests.test-runner` package test.
