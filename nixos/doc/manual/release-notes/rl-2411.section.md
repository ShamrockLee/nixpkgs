...
- `buildPythonPackage` and `buildPythonApplication` now supports both `__structuredAttrs = true` and `__structuredAttrs = false`.

  The representation of the flags attributes as shell/environment variables for most Python building setup hooks are the same as `stdenv.mkDerivation` and other build helpers -- they are space-separated environment variables when `__structuredAttrs = false` and Bash arrays when `__structuredAttrs = true`, and are concatenated to the command without Bash-evaluation. Still, several exceptions exist due to histrical reasons and compatibility purposes:

  - `pytestFlagsArrays` for `pytestCheckHook` and `unittestFlagsArray` for `unittestCheckHook`:
    `builtins.derivaiton` converts them to space-separated environment variables when `__structuredAttrs = false`, but they were mistakenly used as Bash arrays in all other parts of Nixpkgs due to their naming. After this refaction, they are transparently converted to Bash arrays when the setup hooks get sourced.

  - `pytestCheckHook`, `setuptoolsBuildHook` and `unittestCheckHook` Bash-evaluated the command string before execution, and a lot of packages and build helpers (ab)used  this behaviour to quote space-containing flags and to refer to other Bash variables. - A new display-manager `services.displayManager.ly` was added. This refaction preserves this behavior (using the `declare` Bash command) only when `__structuredAttrs = false`.
